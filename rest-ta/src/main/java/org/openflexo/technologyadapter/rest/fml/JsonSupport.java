...
import org.openflexo.technologyadapter.rest.model.ContentSupport;
...
public class JsonSupport implements ContentSupport<JsonResponse> {
	...
	private final RestVirtualModelInstance owner;
	...
	public JsonSupport(RestVirtualModelInstance owner, JsonResponse response) {
		this.owner = owner;
	...
	public JsonSupport(RestVirtualModelInstance owner, String identifier, ObjectNode source, JsonResponse response) {
		this.owner = owner;
		this.response = response;
		this.identifier = identifier;
		this.source = source;
		...
		if (needUpdate() || force) {
			synchronized (this) {
				if (needUpdate() || force) {
					...
					logger.warning("Des trucs a faire la....");
