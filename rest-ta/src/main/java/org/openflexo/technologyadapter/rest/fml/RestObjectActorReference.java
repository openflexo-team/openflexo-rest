...
/**
 * Implements {@link ActorReference} for a {@link RestFlexoConceptInstance}<br>
 * 
 * @author sylvain
 * 
 * @param <T>
...
@XMLElement
public interface RestObjectActorReference extends ActorReference<RestFlexoConceptInstance> {
	...
	abstract class RestObjectActorReferenceImpl extends ActorReferenceImpl<RestFlexoConceptInstance> implements RestObjectActorReference {

		private static final Logger logger = FlexoLogger.getLogger(RestObjectActorReference.class.getPackage().toString());

		private RestFlexoConceptInstance modellingElement;

		@Override
		public void setModellingElement(RestFlexoConceptInstance object) {
			...
			if (object != null) {
				...
				setKey("prout"/*object.getIdentifier()*/);
			}
			else {
				setResourceURI(null);
				setKey(null);
		...
		@Override
		public RestFlexoConceptInstance getModellingElement(boolean forceLoading) {
		...
		public String toString() {
			return "RestObjectActorReference [" + getRoleName() + "] " + Integer.toHexString(hashCode()) + " references "
