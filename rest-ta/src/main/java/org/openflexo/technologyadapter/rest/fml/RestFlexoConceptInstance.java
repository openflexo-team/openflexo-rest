...
@XMLElement
public interface RestFlexoConceptInstance extends ReflectedFlexoConceptInstance<JsonSupport> {
	...
	abstract class RestFlexoConceptInstanceImpl extends ReflectedFlexoConceptInstanceImpl<JsonSupport> implements RestFlexoConceptInstance {
		...
		public <T> T getFlexoPropertyValue(FlexoProperty<T> flexoProperty) {
			...
			if (!fullInformationsHaveBeenRetrieved && getFlexoConcept().getDeclaredProperties().contains(flexoProperty)
					&& (getSupportObject() == null || !getSupportObject().hasValue(flexoProperty.getName()))) {
		...
		protected void retrieveFullInformations() {
			...
			logger.warning("retrieveFullInformations() : Please implement me !");
